...
<script>
	function check() {
		if (document.myform.bSearch.value == "") {
			alert("검색 값을 입력하세요!!");
			return false;
		}
	}
			...
			<div id="content" align="center">
				...
				<table width="99%">
					...
					<tfoot>
						...
						<tr align="center">
							<td colspan="5">
								<a href="search?pgnum=1&bCol=${bCol}&bVal=${bVal}" style="text-decoration: none">${prev}${prev}</a> 
								<a href="search?pgnum=${before}&bCol=${bCol}&bVal=${bVal}" style="text-decoration: none">${prev}</a>
								<c:forEach items="${pg}" var="p">
									<a href="search?pgnum=${p}&bCol=${bCol}&bVal=${bVal}" style="text-decoration: none">${p}</a>
								</c:forEach>
								<a href="search?pgnum=${after}&bCol=${bCol}&bVal=${bVal}" style="text-decoration: none">${next}</a>
								<a href="search?pgnum=${last}&bCol=${bCol}&bVal=${bVal}" style="text-decoration: none">${next}${next}</a></td>
						</tr>
					</tfoot>
				</table>
				<br /> <br />
