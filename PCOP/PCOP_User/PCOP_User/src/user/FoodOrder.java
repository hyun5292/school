...
public class FoodOrder extends JFrame {
	...
	public FoodOrder(String m_id, int s_num) { // 생성자야 실행하면 여기서 초기설정 해줘
		...
		btn_Order.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				if (text_Way.getText().equals("결제방식을 선택하세요")) {
					JOptionPane.showMessageDialog(null, "결제방식을 선택하세요", "오류", JOptionPane.INFORMATION_MESSAGE);
				} else {
					...
					if (message == JOptionPane.YES_OPTION) {
						...
						for (int i = 0; i < tbl_Food.getRowCount(); i++) {
							...
							foDto.setFO_RESULT("접수완료");
							if ((area_Want.getText().toString()).equals("요청사항 입력")) {
								foDto.setFO_MEMO("");
							} else {
						...
						JOptionPane.showMessageDialog(null, "주문이 완료되었습니다", "접수완료", JOptionPane.INFORMATION_MESSAGE);
					}
				}
			}
		});
